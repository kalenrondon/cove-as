    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">
          Recaudado: <span className="font-bold text-green-600 dark:text-green-400">${totalRecaudado.toLocaleString('es-CO')}</span>
          {totalEsperado > 0 && <> · Meta: <span className="font-semibold">${totalEsperado.toLocaleString('es-CO')}</span></>}
        </p>
      </div>
              ...
              <div className="px-4 py-3 flex items-center justify-between bg-gradient-to-r from-primary to-primary-light text-white">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-white/80">{new Date(r.date).toLocaleDateString('es-CO', { year: 'numeric', month: 'long' })} · ${r.amount_per_person.toLocaleString('es-CO')} c/u</p>
                  </div>
                </div>
              </div>
                                  ...
                                  <div className="flex items-center gap-2 shrink-0 ml-2">
                                    {hasPayment && (
                                      <span className={`text-xs font-semibold ${fullyPaid ? 'text-green-600' : 'text-yellow-600 dark:text-yellow-400'}`}>
                                        ${(p.paid_amount || 0).toLocaleString('es-CO')}{fullyPaid ? '' : '/' + r.amount_per_person.toLocaleString('es-CO')}
                                      </span>
                                    )}
